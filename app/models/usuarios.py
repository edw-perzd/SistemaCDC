...
class Toma:
    ...
    def guardar(self):
        if self.id_alumno is None:
            ...
            with mydb.cursor() as cursor:
                sql = "UPDATE toma SET id_taller = %s, fechaInscripcion = %s "
                sql += "WHERE id_alumno = %s"
                val = (self.id_taller, self.fechaInscripcion, self.id_alumno)
                cursor.execute(sql, val)
                mydb.commit()
                return self.id_alumno
            ...
            if result:
                ...
                if result:
                    for taller in result:
                        toma.append(Toma(taller['id_alumno'], taller['id_taller'], taller['nombre_taller'], taller['descrip_taller'], taller['categoria_taller'], taller['fechaInscripcion']))
                    return toma
            else:
